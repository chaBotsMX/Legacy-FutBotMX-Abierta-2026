...
Cams::Cams(HardwareSerial &port_) : port(port_) {}

void Cams::begin(unsigned long baud) {
  port.begin(baud);
}

bool Cams::updatePacketUnitV(int &x, int &y) {
...
bool Cams::updatePacketOpenMV(int &x, int &y, int &goalX, int &goalY, int &goalColor) {
  while (port.available()) {
    uint8_t c = port.read();
