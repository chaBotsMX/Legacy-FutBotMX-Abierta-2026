...
void Omni::move(int angle, int power,int rotation){
  if(angle == BALL_OUT_OF_RANGE){
    float output = constrain((rotation * 8), -165, 165) + (((rotation - prevRot) / 2) * 64);
    prevRot = rotation;
    Serial.println(output);
    motors[0].move(int( output)); //back right
    motors[1].move(int( -output)); //front right
    motors[2].move(int( -output)); //front left
    motors[3].move(int( output)); //back left
  }
  else{
